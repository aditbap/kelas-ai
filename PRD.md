...
**Positioning:** *"From AI Awareness to AI Adoption."* / *"Turn AI from a buzzword into a daily productivity tool for your employees."*
...
4. *(Consultation path: Sales manually creates the Tenant, Subscription, and initial Company Admin account after a deal closes — same downstream state, different entry point.)*
...
Because the core value proposition is that AI adoption continues *after* the onsite session ends, the platform surfaces the Resource Library (tips, prompting templates) proactively rather than only on-demand:
...
| Database | **PostgreSQL** | Relational data (tenants, roles, submissions, grades) fits relational modeling well; mature, well-supported |
| ORM | **Prisma** | Type-safe schema matches the data model above almost 1:1; good AI-agent codegen support |
| Auth | **Auth.js (NextAuth)** or **Clerk**, with tenant/org-aware sessions | Standard session/JWT auth with role + tenantId embedded in session; Clerk if the team wants managed auth + built-in org support, Auth.js if full control is preferred |
...
| Hosting | **Vercel** (app) + managed Postgres (**Neon** or **Supabase**) | Low-ops deployment matched to the Next.js stack |
...
*Escape hatch:* if a future enterprise customer contractually requires physical data isolation, the architecture should support migrating that single tenant to a dedicated schema or database without a full rewrite — keep tenant-scoping logic centralized (not scattered) specifically so this migration stays feasible.
...
- Instructors query across their *assigned* tenants (via `InstructorTenantAssignment`); Super Admin bypasses tenant scoping entirely (with all such access audit-logged).
...
└───────┬─────────────┬─────────────────┬───────────┬──────┘
   ...
   └───────────────────────┬───────────────────────────────┘
                            ▼
   ┌─────────────────────────────────────────────────────┐
   │   PostgreSQL (Prisma)  │  S3 (files)  │  Stripe  │ Email │
   └─────────────────────────────────────────────────────┘
```
...
- **Post-training active usage**: % of employees with platform activity 2/4/8 weeks *after* their cohort's onsite date — this is the core "did adoption stick" signal the business is selling
...
2. **Cohorts are tied to a single onsite date** per training engagement; if a company runs recurring/rolling onsite sessions, the Cohort model may need to support multiple sessions per tenant over time (the current model already allows multiple Cohorts per Tenant, so this should be fine, but confirm whether cohorts ever need to *span* multiple onsite dates).
