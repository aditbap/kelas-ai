...
- [ ] Create the Neon project; create a `dev` branch for local development and a `staging` branch for the staging deploy (no local Docker Postgres needed — everyone connects to their own Neon dev branch).
- [ ] Install and configure Prisma with the Neon serverless driver (`@neondatabase/serverless` + `@prisma/adapter-neon`); connect to the `dev` branch.
- [ ] Connect the Neon Vercel integration so each preview deployment/PR automatically gets its own ephemeral Neon branch.
...
- [ ] Create the Neon `production` branch (sized appropriately — Neon autoscaling/compute settings reviewed, not left on defaults), S3 bucket, and configure production env vars/secrets in Vercel.
